...
bool ConnectionPool::loadConfigFile()
{
	...
	while (!feof(pf))//如果文件存在
	{
		...
		string value = str.substr(idx + 1, endidx - idx - 1);//endidx - idx 个数，下标从0开始，所以-1
...
ConnectionPool::ConnectionPool() {
	...
	//启动一个新的线程，作为来连接的生产者linux thread =>pthread_create
	...
	*/
	shared_ptr<Connection> sp(_connectionQue.front(),
		[&](Connection* pcon) {
			...
			unique_lock<mutex> lock(_queueMutex);
			pcon->refreshAliveTime();//刷新以下开始空闲的起始时间
			_connectionQue.push(pcon);
		});
	_connectionQue.pop();
	cv.notify_all();//消费完连接以后，通知生产者线程检查以下，如果队列为空了，赶紧生产连接
	return sp;
}
